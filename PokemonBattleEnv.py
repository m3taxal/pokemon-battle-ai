...
from custom_encodings import encode_state
...
class PokemonBattleEnv(gym.Env):
    ...
    def pkm_from_id(self, id: int, team: list[BattlingPokemon]) -> BattlingPokemon:
        for pkm in team:
            if pkm.constants.species.id == id:
                return pkm
    ...
    def step(self,
        ...
        # Status stuff
        opp_status_before_turn = sum([1 if pkm.status == Status.NONE else 0 for pkm in opp_team])
        ...
        reward += (dead_opp_pkm_after_turn-dead_opp_pkm_before_turn) / 2 # A maximum of 2 pkm can be killed in a single turn

        # Reward if opp pkm, which did not have status before, has status after turn
        opp_status_after_turn = sum([1 if pkm.status == Status.NONE else 0 for pkm in opp_team])
        reward += max(0, (opp_status_after_turn-opp_status_before_turn)) / 2 # A maximum of 2 pkm can be statused in a single turn

        # Reward if more HP after turn (e.g. if agent healed itself)
        # Will likely be 0 most of the time since moves don't heal that much
        reward += max(0, (own_hp_after_turn-own_hp_before_turn) / own_max_hp)

        # Reward if enemy has lost health
        reward += max(0, (opp_hp_before_turn-opp_hp_after_turn) / opp_max_hp) / 2
